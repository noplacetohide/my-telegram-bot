...
export const TelegramInfoContext = createContext(INIT_VALUE);

export const useTelegramContext = () => {
    const context = useContext(TelegramInfoContext);
    if (context === undefined) {
        throw new Error("TelegramInfoContext must be used within provider");
    }
    return context;
};
export function TelegramProvider({ children }: any) {
    ...
    return (
        <TelegramInfoContext.Provider value={{
            telegramUser
        }}>
            {children}
        </TelegramInfoContext.Provider>
    )
}
